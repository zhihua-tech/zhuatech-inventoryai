...
@Service @Transactional(readOnly=true) public class InventoryAiService {
 private final WorkRecordRepository orders; private final ActivityRecordRepository reports; private final ResourceRegisterRepository resources; private final ReviewRecordRepository reviews; private final CurrentUserService current; /**
                                                                                                                                                                                                                                         * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
                                                                                                                                                                                                                                         */
public InventoryAiService(WorkRecordRepository orders,ActivityRecordRepository reports,ResourceRegisterRepository resources,ReviewRecordRepository reviews,CurrentUserService current){this.orders=orders;this.reports=reports;this.resources=resources;this.reviews=reviews;this.current=current;}
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
...
public List<WorkRecordView> workRecords(){return orders.findAllByOrderByDueDateAsc().stream().map(this::view).toList();}
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Transactional public ReportResult report(Long id,ReportRequest request){WorkRecord item=orders.findById(id).orElseThrow(()->new BusinessException("库存优化任务不存在"));if(item.getStatus()==WorkRecord.Status.COMPLETED)throw new BusinessException("已归档任务不能继续反馈");if(item.getCompletedQty()+request.goodQty()>item.getPlannedQty())throw new BusinessException("完成 SKU 不能超过计划 SKU");item.report(request.goodQty(),request.defectQty());reports.save(new ActivityRecord(item,request.operationName(),request.goodQty(),request.defectQty(),current.get().getFullName(),request.remark()));return new ReportResult(item.getRecordNo(),item.getCompletedQty(),item.getDefectQty(),progress(item),item.getStatus().name());}
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 private Dashboard dashboard(List<WorkRecord> list){int planned=list.stream().mapToInt(WorkRecord::getPlannedQty).sum(),done=list.stream().mapToInt(WorkRecord::getCompletedQty).sum(),exceptions=list.stream().mapToInt(WorkRecord::getDefectQty).sum(),rate=planned==0?0:Math.round(done*100f/planned);List<Metric> metrics=List.of(new Metric("分析 SKU",String.format("%,d",planned),list.size()+" 个组合","blue"),new Metric("完成率",rate+"%",String.format("%,d / %,d",done,planned),"green"),new Metric("库存异常",exceptions+"","缺货或滞销","warn"),new Metric("处置审批",resources.countByStatus(ResourceRegister.Status.ALARM)+"",reviews.countByResult(ReviewRecord.Result.PENDING)+" 项待决策","red"));return new Dashboard(metrics,list.stream().map(this::view).toList(),resources.findAllByOrderByCodeAsc().stream().map(e->new ControlView(e.getCode(),e.getName(),e.getOperatingUnit().getName(),e.getStatus().name(),e.getOee(),e.getLastHeartbeat())).toList(),reviews.findTop10ByOrderByIdDesc().stream().map(i->new ReviewRecordView(i.getReviewRecordNo(),i.getWorkRecord().getRecordNo(),i.getWorkRecord().getSubjectName(),i.getReviewRecordType(),i.getReviewRecordQty(),i.getDefectQty(),i.getResult().name(),i.getInspector())).toList());}
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */

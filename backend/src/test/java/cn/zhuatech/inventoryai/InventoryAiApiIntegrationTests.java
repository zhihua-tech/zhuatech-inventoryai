 ...
 */
@SpringBootTest @AutoConfigureMockMvc class InventoryAiApiIntegrationTests { @Autowired MockMvc mvc; private String token;
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Test void operatorCanOptimizeInventory()throws Exception{mvc.perform(post("/api/ai/inventory/optimize").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"skuCode\":\"SKU-88\",\"onHand\":80,\"inbound\":20,\"averageDailyDemand\":30,\"leadTimeDays\":7,\"daysSinceLastSale\":1,\"shelfLifeRemainingDays\":365,\"marginRate\":0.30}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.action").value("REPLENISH"));}}
